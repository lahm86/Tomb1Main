...
void Lara_Hair_Control(void)
{
    ...
    mesh_base = Object_GetMesh(object->mesh_idx);
    ...
    if (frac) {
        ...
        Matrix_Push_I();
        mesh = &mesh_base[LM_TORSO]; // ignore shotgun
        Matrix_TranslateRel_I(mesh->center.x, mesh->center.y, mesh->center.z);
        Matrix_Interpolate();
...
void Lara_Hair_Draw(void)
{
    ...
    for (int32_t i = 0; i < HAIR_SEGMENTS; i++) {
        ...
        Output_DrawObjectMesh(Object_GetMesh(mesh_idx + i), 1);
